...
const Json2 = (props) => {
  let config = {
      ...
      const test =props.patientList.evolution;
      return (
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.container1}>
              ...
              <Text style={styles.name}>{key2}</Text>
                {Object.keys(test[setNom][key1][key2]).map((key3, i2)=> (
                      <View  >
              <Text style={styles.name}>{key3}</Text>
                {Object.keys(test[setNom][key1][key2][key3]).map((key4, i3)=> (
                      <View style={styles.card} >
              <Text style={styles.name}>{key4}</Text>
                <Text style={styles.count}>{String(test[setNom][key1][key2][key3][key4])}</Text>
            </View>
